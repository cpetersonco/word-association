...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create word-association-68d0
  fly apps create word-association-68d0-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app word-association-68d0
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app word-association-68d0-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'word-association-68d0-staging' does not match app name in config file 'word-association-68d0'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name word-association-68d0-db
  fly postgres attach --app word-association-68d0 word-association-68d0-db

  fly postgres create --name word-association-68d0-staging-db
  fly postgres attach --app word-association-68d0-staging word-association-68d0-staging-db
  ```
